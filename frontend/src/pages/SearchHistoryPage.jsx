...
const SearchHistoryPage = () => {
    ...
    useEffect(() => {
        const getSearchHistory = async () => {
            try {
                const res = await axios.get(`https://netflix-clone-api-xi.vercel.app/api/v1/search/history`);
    ...
    const handleDelete = async (entry) => {
        try{
            await axios.delete(`https://netflix-clone-api-xi.vercel.app/api/v1/search/history/${entry.id}`);
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                    {searchHistory?.map((entry) => (
                        <div key={entry.id} className='bg-gray-800 p-4 rounded flex items-start'>
                            ...
                            <span className={`py-1 px-3 min-w-20 text-center rounded-full text-sm ml-auto ${entry.searchType === "movie"
                                ? "bg-red-600"
                                : entry.searchType === "tv"
                                ? "bg-blue-600"
                                : "bg-green-600"
