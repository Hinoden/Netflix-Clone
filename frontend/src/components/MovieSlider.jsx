...
const MovieSlider = ({category}) => {
    ...
    useEffect(() => {
        const getContent = async () => {
            const res = await axios.get(`https://netflix-clone-api-kohl.vercel.app/api/${contentType}/${category}`);
    ...
    return (
        ...
        >
            ...
            <div className='flex space-x-4 overflow-x-scroll scrollbar-hide' ref={sliderRef}>
                {content.map((item) => (
                    <Link to={`https://netflix-clone-black-two.vercel.app/api/watch/${item.id}`} className='min-w-[250px] relative group' key={item.id}>
