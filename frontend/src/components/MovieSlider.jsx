...
const MovieSlider = ({category}) => {
    ...
    useEffect(() => {
        const getContent = async () => {
            const res = await axios.get(`https://netflix-clone-api-xi.vercel.app/api/v1/${contentType}/${category}`);
    ...
    return (
        ...
        >
            ...
            <div className='flex space-x-4 overflow-x-scroll scrollbar-hide' ref={sliderRef}>
                {content.map((item) => (
                    <Link to={`https://netflix-clone-api-xi.vercel.app/api/v1/watch/${item.id}`} className='min-w-[250px] relative group' key={item.id}>
